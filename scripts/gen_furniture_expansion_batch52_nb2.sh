#!/usr/bin/env bash
# 批次 52：仙气梦幻古风 20 件 — NB2 → game_assets raw
set -euo pipefail
WS="$(cd "$(dirname "$0")/.." && pwd)"
GA="${GAME_ASSETS_HUAHUA:-"$WS/../game_assets/huahua"}"
RAW="$GA/assets/raw/furniture_expansion_batch52"
mkdir -p "$RAW"
GEN="${HOME}/.cursor/skills/gemini-image-gen/scripts/generate_images.py"
MANIFEST="$WS/docs/furniture_expansion_batch52_manifest.json"
...
[[ -f "$MANIFEST" ]] || { echo "run python3 scripts/gen_expansion_batch52_prompts.py first" >&2; exit 1; }
