#!/usr/bin/env bash
# 落地灯/双壁炉/落地钟(原 wallart_winter)/花铲/收纳篮/香薰 — NB2 → rembg → crop → 171
# 断点续跑: ONLY_KEYS="orn_candle" ./scripts/gen_furniture_mixed_refresh_nb2.sh
set -euo pipefail
WS="$(cd "$(dirname "$0")/.." && pwd)"
...
ALL_KEYS=(light_floor orn_fireplace orn_christmas wallart_winter garden_flower_stall orn_pot orn_candle)
if [[ -n "${ONLY_KEYS:-}" ]]; then
