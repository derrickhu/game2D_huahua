#!/usr/bin/env bash
# NB2 生成橡树小屋「春花爬藤」换色房壳 → rembg → 写入 minigame/house
# 布局锁：bg_room_forest_treehouse_oak_nb2.png（同结构，春天树+爬藤花+春屋顶+鸟窝）
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

REF="minigame/subpkg_deco/images/house/bg_room_forest_treehouse_oak_nb2.png"
...
NAME="bg_room_forest_treehouse_spring_bloom_nb2"
PROMPT="house_bg_room_forest_treehouse_spring_bloom_nb2_prompt.txt"
...
mkdir -p "$OUT" "$PREV" "$FINAL" "${GAME_ASSETS_HUAHUA:-../game_assets/huahua}/assets/raw"

raw="$OUT/${NAME}_raw.png"
raw_archive="${GAME_ASSETS_HUAHUA:-../game_assets/huahua}/assets/raw/${NAME}.png"
nobg="$OUT/${NAME}_nobg.png"

echo "== NB2 $NAME (ref: oak treehouse layout) =="
