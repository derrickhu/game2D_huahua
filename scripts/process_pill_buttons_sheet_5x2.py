#!/usr/bin/env python3
"""
切 5×2 胶囊表图 → 每格 rembg (birefnet-general) → crop_trim → 顺时针 90°（与表图竖向胶囊一致时需横过来用）。

输出仅写入仓库外资源库 game_assets（默认 ../game_assets/huahua/assets/...），不写 minigame。

用法（仓库根）:
  python3 scripts/process_pill_buttons_sheet_5x2.py
  python3 scripts/process_pill_buttons_sheet_5x2.py --sheet /path/to/sheet.png
"""
...
DEFAULT_SHEET = os.path.join(
    ...
    "assets",
    "raw",
    "pill_buttons_sheet_5x2_nb2.png",
)
...
def main() -> None:
    ...
    ap.add_argument("--sheet", default=DEFAULT_SHEET, help="5×2 表图 PNG")
    ap.add_argument("-o", "--output-dir", default=DEFAULT_OUT, help="输出目录（仅 game_assets）")
    ...
    with tempfile.TemporaryDirectory(prefix="huahua_pill_5x2_") as td:
        idx = 0
        for row in range(GRID_ROWS):
            for col in range(GRID_COLS):
                ...
                # 5×2 表内多为竖向胶囊，输出顺时针 90° 成横向扁钮
                out_im = out_im.rotate(-90, expand=True, resample=Image.Resampling.BICUBIC)

                name = f"pill_btn_5x2_r{row + 1}c{col + 1}_idx{idx:02d}.png"
