#!/usr/bin/env python3
"""
从 NB2 输出的 `deco_nb2_main_panel_blank_9x16.png` 生成游戏用
`minigame/subpkg_panels/images/ui/decoration_panel_bg_nb2.png`。

默认流程（大面板为**纯白** #FFFFFF 底板）：
...
def _run_rembg_crop_despill(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        arr = np.array(im)
        chroma_magenta_nb2.despill_white_fringe(arr, lum_thresh=234.0, strength=0.65)
        Image.fromarray(arr, "RGBA").save(out, "PNG")


def main() -> None:
    ap = argparse.ArgumentParser(description="Build decoration_panel_bg_nb2.png from NB2 raw.")
    ap.add_argument(
        "--src",
        type=str,
        default=None,
    ...
    default_src = game_assets_dir() / "deco_panel_ui_nb2/for_review/deco_nb2_main_panel_blank_9x16.png"
    src = Path(args.src or default_src)
    if not src.is_file():
