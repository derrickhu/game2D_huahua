#!/usr/bin/env bash
# 扩展批次 50 件家具：NB2 单件出图 → game_assets raw（本脚本仅生图，不入库 minigame）
# 断点续跑: ONLY_KEYS="bf_moon_lantern_stand tea_low_chabudai" ./scripts/gen_furniture_expansion_batch50_nb2.sh
# 跳过已成功: SKIP_EXISTING=1 ./scripts/gen_furniture_expansion_batch50_nb2.sh
set -euo pipefail
WS="$(cd "$(dirname "$0")/.." && pwd)"
GA="${GAME_ASSETS_HUAHUA:-"$WS/../game_assets/huahua"}"
RAW="$GA/assets/raw/furniture_expansion_batch50"
mkdir -p "$RAW"
GEN="${HOME}/.cursor/skills/gemini-image-gen/scripts/generate_images.py"
MANIFEST="$WS/docs/furniture_expansion_batch50_manifest.json"
...
if [[ ! -f "$MANIFEST" ]]; then
  echo "missing manifest: $MANIFEST (run python3 scripts/gen_expansion_batch50_prompts.py first)" >&2
  exit 1
fi
