#!/usr/bin/env python3
"""
图纸商店弹窗壳 v1：NB2 绿幕原图 → 四边洪水去绿 → 轻量 despill → 预览/入库。

用法（仓库根）:
  python3 scripts/process_furniture_workshop_blueprint_shop_shell_v1_nb2.py [raw.png]
"""
...
SRC_DEFAULT = os.path.join(
    REPO, "../game_assets/huahua/assets/raw/furniture_workshop_blueprint_shop_shell_v2_nb2.png"
)
MAX_W = 628
PAD = 8
...
def main() -> int:
    ...
    out_preview = os.path.join(REPO, ".tmp/ui_prototypes", f"{stem}_nobg.png")
    os.makedirs(os.path.dirname(out_preview), exist_ok=True)
    im.save(out_preview, "PNG", optimize=True)
    print(f"OK -> {out_preview} {im.size}")

    game_out = os.path.join(
        REPO, "minigame/subpkg_panels/images/ui/furniture_workshop_blueprint_shop_shell_nb2.png"
    )
    im.save(game_out, "PNG", optimize=True)
    print(f"OK -> {game_out} {im.size}")
    return 0
