#!/usr/bin/env bash
# 茶香小院默认房壳 bg_room_tea_house_nb2：NB2（布局锁蝴蝶小屋 v11）→ rembg → preview + house
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

REF="minigame/subpkg_deco/images/house/preview/bg_room_butterfly_house_default_v11_1k_nb2.png"
OUT="${GAME_ASSETS_HUAHUA:-../game_assets/huahua}/assets/preview_house_room"
PR="docs/prompt/house_bg_room_tea_house_nb2_prompt.txt"
NAME="bg_room_butterfly_house_xianqi_nb2"
...
[[ -f "$REF" ]] || { echo "missing layout ref $REF" >&2; exit 1; }
...
echo "== NB2 $NAME (layout ref: butterfly v11) =="
ok=0
