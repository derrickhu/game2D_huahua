#!/usr/bin/env python3
"""
成长之路面板壳：NB2 绿幕原图 → 四边洪水去绿（保留完整外圈紫框）→ 轻量绿边 despill → 入库。

UI 整页壳体勿用 rembg 缩 alpha：会把外圈紫框当背景吃掉。
绿幕仅在外围，洪水抠图即可；只做颜色去绿，不侵蚀透明边。

与 process_furniture_workshop_panel_shell_nb2.py 同一套算法，仅换输入/输出路径。

用法（仓库根）:
  python3 scripts/process_growth_panel_shell_nb2.py [raw.png]
"""
...
SRC_DEFAULT = os.path.join(REPO, "../game_assets/huahua/assets/raw/growth_panel_shell_v3_2k_nb2.png")
OUT = os.path.join(REPO, "minigame/subpkg_panels/images/ui/growth_panel_shell_nb2.png")
MAX_W = 680
PAD = 8
...
def is_green_screen(rgb: np.ndarray, *, dist_cutoff: float = 72.0) -> np.ndarray:
    """绿幕像素判定：仅用于从画布边缘 flood，勿扩大以免吃进紫框与薄荷叶饰。"""
    r = rgb[:, :, 0]
    g = rgb[:, :, 1]
    b = rgb[:, :, 2]
...
def despill_green_fringe(rgba: np.ndarray, *, strength: float = 0.5) -> None:
    """只改 RGB 去绿，不动 alpha。"""
