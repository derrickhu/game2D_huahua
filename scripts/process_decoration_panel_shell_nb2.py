#!/usr/bin/env python3
"""
家具装修面板底栏壳：NB2 绿幕原图 → 四边洪水去绿 → 轻量绿边 despill → 入库。

与 process_dressup_panel_shell_nb2.py 同管线（缩放后再压一遍绿边）。

UI 整页壳体勿用 rembg：会把外圈玫瑰框当背景吃掉。

用法（仓库根）:
  python3 scripts/process_decoration_panel_shell_nb2.py [raw.png]
  python3 scripts/process_decoration_panel_shell_nb2.py --out /path/to/preview.png raw.png
"""
...
SRC_DEFAULT = os.path.join(REPO, ".tmp/ui_prototypes/decoration_panel_shell_nb2.png")
OUT = os.path.join(REPO, "minigame/subpkg_panels/images/ui/decoration_panel_shell_nb2.png")
MAX_W = 710
PAD = 8
...
def process(src: str, out: str) -> None:
    ...
    print(f"OK {src} -> {out} ({im.size[0]}x{im.size[1]})")


def main() -> None:
    ap = argparse.ArgumentParser(description="Process decoration panel bottom-sheet shell NB2 raw PNG.")
    ap.add_argument("src", nargs="?", default=SRC_DEFAULT, help="Raw NB2 PNG (green screen)")
    ap.add_argument("--out", "-o", default=OUT, help="Output transparent PNG")
    args = ap.parse_args()
