#!/usr/bin/env python3
"""
冷饮工具线（mixer）：Gemini 1×3 sheet（16:9）→ 切三列（列内缩）→ 品红抠图 → trim → minigame/images/tools/mixer/

前置：docs/prompt/tool_line_mixer_nb2_prompt.txt

用法：
  python3 scripts/build_tool_mixer_pilot.py
  python3 scripts/build_tool_mixer_pilot.py --sheet path/to.png
"""
...
PROMPT_FILE = _REPO / "docs" / "prompt" / "tool_line_mixer_nb2_prompt.txt"
ASSETS = Path("/Users/huyi/rosa_games/game_assets/huahua/assets")
REVIEW_DIR = ASSETS / "tool_lines" / "for_review"
SHEET_OUT = REVIEW_DIR / "tool_mixer_sheet_16x9.png"
DEST_DIR = _REPO / "minigame" / "images" / "tools" / "mixer"
PREFIX = "tool_mixer"
...
def _generate_three_squares() -> None:
    prompts = [
        "1:1 merge-game icon. Frosted glass measuring cup, steel blue-gray tint, stainless pour lip, faint etched tick marks only NO readable numbers. Fills 88-92 percent of square both ways. Heavy 2D. Background ONLY flat #FF00FF. ZERO text.",
        "1:1 merge-game icon. Tall frosted glass mixing beaker OR steel-blue cocktail shaker with simple metal cap band, bar workshop. Large in 1x1 frame, chunky silhouette. Heavy 2D. Background ONLY flat #FF00FF. ZERO text.",
        "Cute hand-painted 2D merge game icon 1x1. Small cold drink bar counter: dark gray top, brushed steel front, mute syrup bottle shapes with NO labels. Wide squat isometric, object very large in frame, small margins. Single flat magenta background, no gradient. No text.",
    ]
    paths: list[Path] = []
