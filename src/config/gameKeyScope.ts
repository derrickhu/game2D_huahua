 ...
 *
 * - 微信 / H5：huahua_{suffix}（与已上线版本逐字一致，不得改动）
 * - 抖音：      huahua_tt_{suffix}
 *
 * 隔离范围：本地存储 key、云端集合、经分 gameKey。
 * 不隔离：CloudBase HTTP 路由前缀（/huahua-api），两端共用同一套云函数，由 JWT 里的 plt 分流。
 */
import {
...
/** 存档 / 集合 / 经分 gameKey 使用的命名空间：huahua 或 huahua_tt */
...
/**
 * 把业务里写死的 `huahua_xxx` 存储 key 映射到当前平台命名空间。
 *
...
export function scopeStorageKey(
  ...
  // 第三方 SDK（如经分）自带前缀的 key 也纳入隔离，避免两端共用匿名 ID
